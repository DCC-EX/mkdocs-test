...
Note: Switches are not suitable for controlling turnouts/points because the command station cant reach out a finger and change the switch position when the turnout/point is changed from a throttle or some other EXRAIL script. (Unless of course, you are into animating a servo to do just that!)
