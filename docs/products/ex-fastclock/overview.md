...
EX-FastClock is a standalone adjustable speed clock built using an additional Arduino Uno microcontroller and a plug-in shield type TFT touch screen.  The commands built into the **DCC-EX EX-CommandStation** make it possible to also integrate an existing microcontroller based fast clock.
...
This project is based on a project originally written by Jim Gifford (Hallet Cove Southern) in June 2017.  This project used a 32 x 8 LED matrix to display the time and was controlled by a number of push buttons.
...
This project has now been extended to add the capability of integrating the clock to the **DCC-EX EX-CommandStation** and more specifically the **EXRAIL** automation feature which now allows time based control. This feature is optional and can be controlled by the config.h file.
