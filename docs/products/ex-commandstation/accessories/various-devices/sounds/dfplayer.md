...
Because the DFPlayer is a Serial device, this can be inconvenient for **EX-CommandStation** with limited serial pins available (eg **EX-CSB1**) so **DCC-EX** provides an additional HAL driver to access the device over I2C with a suitable UART breakout board.
...
The DFPlayer implementation require a directory named 01 to 99, and should contain all the audio files named 001.mp3 to 255.mp3. The format `001<sound title>.mp3` is also supported.
...
The **EXRAIL** [cookbooks](/products/ex-commandstation/exrail/cookbooks/index.md) provide several worked example sound scenarios.
