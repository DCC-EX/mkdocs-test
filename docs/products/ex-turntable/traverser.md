...
![Heljan Traverser](../../_static/images/ex-turntable/heljan-traverser.jpg){ width=400px }
<br/>A horizontal traverser by Heljan
...
The same components outlined in [what you need for ex-turntable](purchasing.md#what-you-need-for-ex-turntable) apply in traverser mode, with two likely exceptions:

1. Home and limit sensors: Given the need for positive stops at each end's limit of travel, be it vertical, horizontal, or restricting rotation within an arc for limited rotation turntables, micro switches are likely a better choice for these. <br/>
![Micro switch](../../_static/images/ex-turntable/micro-switch.png){ width=400px }
...
General assembly is as per the [assembly](assembly.md) page, with the aforementioned limit sensor being connected to the Arduino's pin 2.
...
![Traverser Assembly Fritzing](../../_static/images/ex-turntable/traverser-assembly.png) { width=400px }
...
| ULN2003 \+ | 5V | A0 V |
| ULN2003 \- | GND | A0 G |
| Limit sensor common | GND | 2 G |
| Limit sensor normally open (NO) | 2 | 2 S |
| Home sensor common | GND | 5 G |
| Home sensor normally open (NO) | 5 | 5 S |
...
Prepare to load the software as per [load the ex-turntable software](assembly.md#7-load-the-ex-turntable-software), and ensure "config.h" has been created by copying or renaming "config.example.h".
...
!!! note  "Tip"
    ...
    Refer to [sanity steps](configure.md#sanity_steps).
...
To enable SENSOR_TESTING mode, you will need to edit "config.h" again, and this time remove the "//" from the beginning of the `#define SENSOR_TESTING`` line (Refer to [sensor testing](configure.md#sensor_testing) to enable sensor testing mode.):
...
!!! note "The HOME sensor must be activated"
