...
Serial command to manage in-command-station Consists/Multiple Unit Trains (MUs).  These are created and held temporarily in the **EX-CommandStation** and are lost when the command station restarts.
...
* **leadLoco**: Notional address of the loco to become the lead loco. (1-10293``) <br/>This does not need to be a real loco.  This will be the address that you select with the throttle or **EXRAIL** sequence to control all the locos in the consist/Multiple Unit train at the same time.
