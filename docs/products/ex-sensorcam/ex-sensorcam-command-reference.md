...
| **`r%%`**<br>r12 | `<Nr %%>` | **Refresh reference image for sensor**<br>enAbles sensor & capture new reference. **r00** for ALL |
...
| **`t1,%%`**<br>t1,12 | `<Nt 1 %%>` | **Trash pvtThresholds for S%% bank**<br>clears 1 bank of 8 pvtThresholds. **t1,99** clears ALL pvtT. |
...
| **`w`**<br> w | `<Nw>` | **Wait for command. &nbsp; (NOTE: 't1' for alternative action)** <br>suspends image capture & scrolling. Waits for 'Enter' |
| **``x  y  z``** | `<n/a>` | **Reserved commands for image transfer management**<br>Sends image data to USB port for Processing4 (X Y Z) |
| n/a | `<N[M #00]>` | **Show Current CAM selection and others available** <br>[ # allows switching current cam to CAM # (1-4) |
...
| **`R`** | `<n/a>` | **Reset EX-SensorCAM** (CS <NR\> equates to <Nr 00\>) <br>Reset into EX-SensorCAM mode, exit any WiFi mode |
| **`&`** | `<n/a>` | **Print statistics since last '&' cmd.** <br>USB histogram of trips + potential trips of 1-3 frames |
...
| **`@##`** | <na> | **change scroll trip symbol - refer to manual** |
