...
Multiplexors are handled automatically by **DCC-EX** and all drivers for i2c devices can take a multiplexed address when being defined.

When the command station starts, it scans the i2c network for all devices it can find, including those behind one or more multiplexors. Although the list can only guess the device type (there is no automatic device recognition, nor rigid standards that associate addresses with device types) it will give the fully qualified I2C address of each device and it is this that must be copied to the HAL device definition.
...
So if you have two devices with the same address they must BOTH be behind separate multiplexor channels.
