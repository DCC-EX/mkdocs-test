...
The following Stash  commands are available:
 ...
 | IFSTASH(stashId) | N/A | True if stash element stashId is not zero |
 | IFSTASHED_HERE(stashId) | N/A | True if stash element stashId contains the current tasks loco |
 | N/A | `<JM>` | query all stashes (returns `<jM stashId loco>` where loco is not zero) |
 | N/A | `<JM stashId>` | Query loco in stash (returns `<jM stashId loco>`) |
