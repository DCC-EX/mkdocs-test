...
Varuous screens can be accessed via the dropdown menu or by swiping left and right.
...
- read the decoder’s DCC Address. (this reads multiple CVs to obtain the driveable address, which may be a consist)
  Click the Read button on the DCC Address row.

- write a new DCC Address to the decoder (This writes multiple CVs to automatically manage long and short addresses)
...
- Measure the distance between the sensors.

- Enter the two sensor numbers on the speedometer screen.

- Enter the distance between the sensors, and select the Scale.

- Put the loco you want to measure on the track before the first sensor.

- Start the loco moving.
...
- Allow access if the device/phone asks.
...
- Browse down to the folder …Internal shared storageAndroiddatadcc_ex.ex_toolboxfiles
