...
To get a more in depth introduction to the DCC standard and protocol, you can read the [Wikipedia](https://en.wikipedia.org/wiki/Digital_Command_Control) and [DCC Wiki](https://dccwiki.com/Introduction_to_DCC) introduction pages.
...
![PWM Waveform Image](../../_static/images/power/sparkfun_pwm.png){ width=300px }
<br/>PWM Waveform Image.

**Frequency of PWM in DC Operation** - PWM can use a variety of frequencies for the pulses it sends, and this can alter motor behaviour and noise etc. The default frequency used for the **EX-CSB1** is xxHz, but this can be varied using the virtual DCC functions 28-31 (check) to allow you to alter the frequency to better suit your loco's motor. This can of course be done during running from the throttle.

The DC PWM frequency can be set using virtual DCC Functions 28-31

### Managing DCC and DC Modes in TrackManager

If you are ready to dive into customising your track outputs, **TrackManager** is the tool you'll use. It allows you to easily switch between DCC and DC modes for any track connected to your **EX-CommandStation**.
...
For detailed steps on how to use TrackManager to change track modes and settings, check out the [TrackManager page](../../products/ex-commandstation/trackmanager/index.md).
...
Note the **EX-CSB1** *has* a brake pin and *is* therefore capable of supporting DC PWM.

!!! warning inline end "Never drive from/to DC to DCC"
...
Some suggested precautions are to add 4 fuses on wires (-b +b, -a +a) to the **EX-CommandStation** connections. Use 2A fuses for the **EX-CSB1** or 5A fuses for the larger motor drivers.
...
**EX-CommandStation** **production version 5.0+** supports both DCC (PWM) and DC (PWM) Pulse Width Modulation modes as an *embedded standard feature*. <br/>
...
An Arduino Mega (with or without WiFi) and **EX-CSB1**, **EX-CommandStation** has two ready to run Tracks (**A** & **B**) which can be configured as:
...
You will instead be using a regulated DC (Laptop) 12-16v DC 3-5Amp power supply <sup>1</sup> to your motor shield and or Motor Board (booster) to run all your DCC Locos and analogue DC Cabs on all your individual tracks/districts/blocks.
...
![DCC Waveform](../../_static/images/trackmanager/dcc.svg){ width=300px }
<br/>What a DCC waveform looks like

![PWM Waveform](../../_static/images/trackmanager/pwm.svg){ width=300px }
<br/>What a PWM waveform looks like
...
![DCC PWM signal](../../_static/images/trackmanager/dcc-and-dc-on-track.png){ width=300px }
<br/>DCC PWM signal
...
This is all done through a single **EX-CommandStation**.
...
!!! note "Do not use one of your existing loco DCC addresses"
...
!!! warning "The motor driver must have a brake pin defined"

    In order for **TrackManager** to operate in DC mode, the motor driver must have a brake pin defined in your "config.h" motor driver definition.
  
    There are a very limited number of motor drivers that are known to be suitable for use with DC mode. The current list is available in ==TODO== :ref:`reference/hardware/motor-boards:trackmanager dc compatible boards`. Any other motor drivers, not explicitly listed, are not supported. 
 
    Note the **EX-CSB1** and **EX-8874** *have* a brake pin and *is* therefore capable of supporting DC PWM.

Do not attempt to connect two insulated tracks together and drive a DCC engine back and forth until someone bolder than you tries it first (or you've tested it).

!!! warning "Never drive a loco from/to DCC to DC.
